...
def build_background(image_paths, beat_times, voice_dur, tmpdir, visual_style=DEFAULT_STYLE):
    """Dead simple background — scale images, concat, done. Never crashes."""
    print("Building background...")
    bg = f"{tmpdir}/bg.mp4"

    # No images — dark gradient
    if not image_paths:
...
def assemble_video(word_timings, hook_word, audio_path, bg_path,
    ...
    while i < len(word_timings):
        group = word_timings[i:i+5]
        if group:
            text  = " ".join(w[0] for w in group)
            t0    = group[0][1] + HOOK_DUR
            t1    = group[-1][2] + HOOK_DUR
            if i + 5 < len(word_timings):
                t1 = min(t1 + 0.04, word_timings[i+5][1] + HOOK_DUR)
